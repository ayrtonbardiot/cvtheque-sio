        ...
        <div class="w-full max-w-full px-3 mt-6 md:w-7/12 md:flex-none">
            <div class="relative flex flex-col min-w-0 break-words bg-white border-0 shadow-soft-xl rounded-2xl bg-clip-border">
                <div class="p-6 px-4 pb-0 mb-0 bg-white border-b-0 rounded-t-2xl">
                    <h6 class="mb-0">Fiche édition métier</h6>
                </div>
                <div class="flex-auto p-4 pt-6">
                    <div class="flex-auto p-2">
                        <form role="form" method="post" action="{{ route('metiers.update', ['metier' => $metier->id]) }}">
                            @method('PUT')
                            @csrf
                            ...
                            <div class="mb-4">
                                <input type="text"
                                       ...
                                       placeholder="Nom du métier" aria-label="Nom du métier" value="{{old('libelle', $metier->libelle)}}"/>
                                @error('libelle')
                            ...
                            <div class="mb-4">
                                <input type="text"
                                       ...
                                       placeholder="Slug" aria-label="Slug" value="{{old('slug', $metier->slug)}}"/>
                                @error('slug')
                                <p class="text-red-500" role="alert">{{ $message }}</p>
                                @enderror
                            </div>
